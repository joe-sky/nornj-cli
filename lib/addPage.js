  ...
  inquirer
    ...
    .then(answers => {
      co(function*() {
        ...
        //Mock server route files
        renderFile(
          `./templates/mock/routes/newMockRoute${pageTemplate}.js.nornj`,
          `./mock/routes/${pageName}.js`,
          templateData,
          templateDataGlobal
        );
        renderFile(`./mock/app.js`, null, {
          delimiters: {
            start: '/{',
            end: '}/'
          },
          pages: nj.render(fs.readFileSync(`./templates/mock/app.js.nornj`, 'utf-8'), { pageName }, templateData, templateDataGlobal)
        });
        ...
        const tmplsRootStore = includeParser(
          fs.readFileSync(`./templates/src/stores/root.${n`${templateDataGlobal}.mobxRoot` ? 'store' : 'mst'}.js.nornj`, 'utf-8'),
          null,
          nj.tmplRule,
          true
        );
        renderFile(
          `./src/stores/root.${n`${templateDataGlobal}.mobxRoot` ? 'store' : 'mst'}.js`,
          null,
          createParamsByTemplates(
            tmplsRootStore,
            {
