  ...
  if (templateMultiPage) {
    ...
    inquirer.prompt([{
      ...
      co(function*() {
        ...
        //page resource files
        const pageFiles = fs.readdirSync(`./templates/src/web/pages/newPage${pageType}`);
        pageFiles.forEach(fileName => renderFile(`./templates/src/web/pages/newPage${pageType}/${fileName}`, `./src/web/pages/${pageName}/${fileName.startsWith('newPage.') ? pageName + fileName.substr(fileName.indexOf('.')) : fileName}`, { pageName }));
        ...
        renderFile(`./routes-web.js`, null, {
          ...
          clear: pkg.njCliConfig.useJsx ? /\{\/\*__extraComment__\*\/\}/g : /<!--__extraComment__-->/g
        });
