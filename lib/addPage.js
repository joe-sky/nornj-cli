...
const pageCreat = (pageType, pageName) => {
  console.log(`newPage${pageType}`);
  //server route files
...
function normal(pageName) {
  const questions = [{
    type: 'list',
    name: 'type',
    message: 'what do you want to generate page?',
    choices: [
      'page',
      'pageEmpty',
      'pageChart',
  ...
  inquirer.prompt(questions).then(function (answers) {
    switch (answers.type) {
      case 'page':
        ...
        break;
      case 'pageEmpty':
        pageCreat('Empty', pageName)
        break;
      case 'pageChart':
        pageCreat('Chart', pageName)
        break;
      default:
        break;
    }
