...
function cascadeRenderTemplateFile(path, renderTo, type, ...params) {
  let files = [];
  if (fs.existsSync(path)) {
    files = fs.readdirSync(path);
    files.forEach(function(file, index) {
      ...
      if (fs.statSync(curPath).isDirectory()) {
        cascadeRenderTemplateFile(curPath, renderTo, type, ...params);
      } else {
        const newPath = nj.render(curPath.split(`${sep}templates${sep}${type}${sep}`)[1], ...params);
        renderFile(
          curPath,
          `${renderTo}${sep}${newPath.endsWith('.nornj') ? newPath.replace(/\.nornj/, '') : newPath}`,
          ...params
        );
      }
    });
