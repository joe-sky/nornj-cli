...
const { sameNameExts, insertTmplDelimiters, NOTHING, TEMPLATES_DIR } = require('./constants');
...
function renderFile(path, pathTo, ...params) {
  ...
  pathTo = pathTo != null ? pathTo : path;
  const lastIndexPoint = pathTo.lastIndexOf('.');
  const ext = pathTo.substr(lastIndexPoint);
  if (
    ['.ts', '.tsx'].indexOf(pathTo.substr(lastIndexPoint)) >= 0 &&
...
function cascadeRenderTemplateFile(templatesPath, path, renderTo, type, templateName, ...params) {
  ...
  if (fs.existsSync(path)) {
    ...
    files.forEach((file, index) => {
      ...
      if (fs.statSync(curPath).isDirectory()) {
        ...
        cascadeRenderTemplateFile(templatesPath, curPath, renderTo, type, templateName, ...params);
      } else {
