...
module.exports = (command = 'install') => {
  co(function*() {
    ...
    hasPrivateLib && execSync(`npm ${command == 'update' ? 'install' : command} ${command == 'install' ? pkg.njCliConfig.privateLib : pkgDeps.privateDeps.join(' ')} --registry ${pkg.njCliConfig.privateNpmUrl} --save-dev`, { stdio: [0, 1, 2] });
