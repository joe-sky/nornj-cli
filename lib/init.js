...
function templateType(projectName) {
  const questions = [{
    type: 'list',
    name: 'type',
    message: 'Please select a template framework',
    choices: [
      'react-mobx',
      'react-mst',
    ],
  ...
  inquirer.prompt(questions).then(function (answers) {
    switch (answers.type) {
      case 'react-mobx':
        initTmp('react-mobx', projectName)
        break;
      case 'react-mst':
        initTmp('react-mobx', projectName)
        break;
...
function initTmp(tmplName, projectName) {
  /**
   * 判断是否使用gitHub模版
   */
