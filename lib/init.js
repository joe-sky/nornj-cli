...
const { getTemplatePath, getRootPath, deleteFolder, renderFile, CNPM_REGISTRY } = require('./utils');
const nj = require('nornj/dist/nornj.common').default;
require('./nj.config');
const inquirer = require('inquirer');
const argv = require('yargs').argv;

function initProjectTemplate(projectName) {
  co(function*() {
    let jest = '';
    let puppeteer = '';
...
`;
    ...
    renderFile(`./${projectName}/package.json.nornj`, `./${projectName}/package.json`, {
      jest,
      puppeteer
    });
    fs.unlinkSync(`./${projectName}/package.json.nornj`);
    ...
    .prompt([
      {
        ...
        choices: ['react-mst', 'react-mst-app', 'other']
      }
    ])
    .then(answers => {
      co(function*() {
        let templateName = answers.templateName.replace(/\[[^[\]]+\]/g, '');
        ...
        if (templateName.indexOf('/') > -1) {
          ...
          download(templateName, projectName, function(error) {
            ...
            initProjectTemplate(projectName);
          });
        } else {
          //Use local templates
