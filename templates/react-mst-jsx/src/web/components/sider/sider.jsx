...
export default class Sider extends Component {
  ...
  render() {
    ...
    const generateMenu = items => items.map(item => <if condition={item.type == 'item'}>
      <Link key={item.index} to={item.link} onClick={this.selectMenu(item.index)} className={`menu-item ${store.sider.current.toLowerCase() == item.index.toLowerCase() ? 'cur' : ''}`}>
        {item.name}
      </Link>
      <elseif condition={item.type == 'group'}>
        <div key={item.name} className="menu-group">
          <div className="menu-tit" onClick={this.toggleMenu(item)}><Icon type="book" style={{ marginRight: 3 }} />{item.name}</div>
          <div className={`menu-cnt ${item.expanded ? 'expanded' : ''}`} style={{ height: item.expanded ? item.children.length * 40 : 0 }}>
    ...
    return (
      <Menu pageWrapId="page-wrap" outerContainerId="outer-container" onStateChange={this.isMenuOpen} isOpen={store.sider.isOpen} width={200} noOverlay>
        <style jsx global>{`
          .bm-menu-wrap {
        ...
        `}</style>
        <div className="site-logo"><div className="vicb-logo" /></div>
        <div className="site-menu-tit"></div>
        <div className="site-menu-cnt">
          {menuCnt}
        </div>
      </Menu>
    );
