...
export default class Page1_1 extends Component {
  ...
  initTree() {
    this.setState({
      expandedKeys: this.getExpandedKeys(toJS(this.props.store.page1_1.menuData)),
  ...
  @autobind
  onSaveAddPermission() {
    this.props.store.page1_1.saveRolePermission({
      roleId: this.props.store.page1_1.addRoleId,
      menuIds: this.props.store.page1_1.menuIds
  ...
  componentDidMount() {
    ...
    Promise.all([
      ...
      this.props.store.page1_1.getRoleMenuTree().then(() => this.initTree()),
    ]).then(() => {
      closeLoading()
    });
  }
  ...
  onEdit(record, index) {
    ...
    this.props.store.page1_1.setDisable(false);

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      this.props.store.page1_1.getRoleMenuTree({ roleId: record.roleId }).then(() => this.initTree()),
    ]).then(() => {
      closeLoading()
    });
  }
  ...
  render() {
    ...
    const { store: { page1_1 } } = this.props;
    return tmpls.page1_1({ components: { 'ant-TextArea': Input.TextArea } }, this.state, this.props, this, {
      styles,
      page1_1,
      loop,
